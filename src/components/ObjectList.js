...
import { View, Text, TouchableOpacity, Image, FlatList, StyleSheet, Modal, Animated } from 'react-native';
...
const ObjectList = ({ route, navigation }) => {
    ...
    const fetchObjects = async () => {
        try {
            ...
            const response = await fetch(`http://192.168.0.157:3000/getObjectsByCategory/${category.id}`);
    ...
    const handleAddObject = () => {
        ...
        navigation.navigate('AddObject', {
            ...
            parentObjectID: null, // No parent for top-level objects
            onObjectAdd: handleObjectAdd,
        });
        console.log('Navigating to AddObject with category:', category);
    };
        ...
        try {
            const response = await fetch('http://192.168.0.157:3000/addObject', {
                ...
                body: JSON.stringify({
                    ...
                    AudioUri: newObject.AudioUri, // Include if applicable
                    ParentObjectID: newObject.ParentObjectID, // Include parent ID
                }),
            });
        ...
        return (
            <View key={parentObject.id}>
                <TouchableOpacity
                    ...
                    onPress={() => handleObjectsPress(parentObject)}
                >
    ...
    const handleObjectsPress = (parentObject) => {
        if (parentObject) {
